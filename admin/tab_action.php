...
if (isset($_POST['registerbtn'])) {
    ...
    if (isset($_FILES['icon']) && !empty($_FILES['icon']['name'])) {
        ...
        if (move_uploaded_file($image_tmp, $image_folder)) {
            ...
            $query = "INSERT INTO tab (name,url,icon) VALUES ('$name','$url','$unique_image_name')";
...
if (isset($_POST['delete_btn'])) {
    if (isset($_POST['delete_id'])) {
        ...
        $query = "SELECT icon FROM tab WHERE id = '$delete_id'";
        $result = mysqli_query($conn, $query);
        if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $image_path = "upload/" . $row['icon'];

            // Delete carousel from database
            $delete_query = "DELETE FROM tab WHERE id = '$delete_id'";
...
if (isset($_POST['update_btn'])) {
    ...
    if (!empty($_FILES['icon']['name'])) {
        ...
        $query = "SELECT icon FROM tab WHERE id = '$carousel_id'";
        $result = mysqli_query($conn, $query);
        if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $old_image_path = "upload/" . $row['icon'];
        ...
        if (move_uploaded_file($image_tmp_name, $image_path)) {
            ...
            $query = "UPDATE tab SET icon='$unique_image_name' WHERE id=$carousel_id";
        } else {
        ...
        $query = "UPDATE tab SET icon='$image_name' WHERE id=$carousel_id";
    }

    if (mysqli_query($conn, $query)) {
        $_SESSION['message'] = "Carousel updated successfully!";
        header("Location: tab.php"); // Redirect back to carousel list
        exit;
    } else {
